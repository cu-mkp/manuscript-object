...
for i in range(0, 340):
    ...
    folio = manuscript_version + "/" + manuscript_version + "_p" + number + side +"_preTEI"
    input_filename = ms_xml_path + folio + ".xml"
    ...
    for block in tree.findall(".//ab"):
        ...
        for item in block.findall(".//" + tag):
            ...
            #if tag_text[-1] == "s":
                # remove all plural 's' to avoid issues later on
                #tag_text = tag_text[:-1]
            #    plural = True
            #else:
            #    plural = False
            ...
            # and might also be stuck with other words
